...
U16WALLETLINK="https://github.com/proximity-cash/proximity/releases/download/1.0.2.2.fix/linux-daemon.tar.gz" 		#for xenial version (16.04), leave it empty if not supported
U17WALLETLINK="" 		#for artful version (17.10), leave it empty if not supported
U18WALLETLINK="" 		#for bionic version (18.04), leave it empty if not supported
DE8WALLETLINK=""             #for jessie version (Debian 8), leave it empty if not supported
DE9WALLETLINK=""            		#for stretch version (Debian 9), leave it empty if not supported
CO7WALLETLINK=""             #for Core version (CentOS 7), leave it empty if not supported
F27WALLETLINK=""					#for Fedora 27, leave it empty if not supported
F28WALLETLINK=""			#for Fedora 28, leave it empty if not supported
F29WALLETLINK=""					#for Fedora 29, leave it empty if not supported
#
BLOCKDUMPLINK="" 																					#link blockchain cache/bootstrap
BOOTSTRAP="false"																				 	#selector (bootstrap=true/block cache=false)
#
WALLETDIR="prox"                                                             #wallet instalation directory name
DATADIRNAME=".prox"                                                  #datadir name
CONFFILENAME="prox.conf"                                                     #conf file name
DAEMONFILE="proxd"                                                           #daemon file name
CLIFILE="prox-cli"                                                           #cli file name
P2PPORT="23500"                                                                 #P2P port number
RPCPORT="23501"                                                                 #RPC port number
IPV6SUPPORTED="true"															#IPv6 support by coin wallet (true -  supported; false - not supported)
COLLAMOUNT="4000"                                                               #collateral amount
TICKER="PROX"                                                                    #coin ticker / symbol
#
...
function print_welcome() {
	...
	echo "╔══════════════════════════════════════════════════════════════════════════════╗"
	echo "║                                                                              ║"
	echo "║                 Welcome to PROX masternode autosetup script                  ║"
	echo "║                                                                              ║"
	echo -en "║                 Version: ${VERSION}\r"
	echo -en "\033[45C${ITA}${NC}                   ║\n"
	echo "║                 Script support: https://discordapp.com/invite/XEcrE2Y        ║"
	echo "║                                                                              ║"
	echo "╚══════════════════════════════════════════════════════════════════════════════╝"
	echo
	...
	if [ ${#tcp4ports[@]} -gt 0 ]; then
		for tcp4port in "${tcp4ports[@]}"; do
			if ! [[ "$tcp4port" =~ "'('"${P2PPORT}"'|'"${RPCPORT}"')'" ]] ; then 
				portlist+=($tcp4port); 
				if ! [ "$tcp4port" = "22" ]; then notonly22=1; fi
			fi
		done
	fi
...
function install_prerequisites() {
	...
	if [ $osver = "trusty" ]; then
		...
		sudo apt-get install unzip curl build-essential libtool autotools-dev libssl-dev libevent-dev bsdmainutils libboost-all-dev libminiupnpc-dev libdb4.8-dev libdb4.8++-dev -y >>${LOGFILE} 2>&1
		[ $? -eq 0 ] && ec=0 || ec=1
		[ $ec -eq 0 ] && echo -en $STATUS0 || echo -en $STATUS1
		...
		echo -en " Installing required packages \r"
		sudo apt-get update >>${LOGFILE} 2>&1
		sudo apt-get install unzip curl build-essential libtool autotools-dev libssl-dev libzmq3-dev libminiupnpc-dev libevent-dev bsdmainutils libboost-all-dev libminiupnpc-dev libdb4.8-dev libdb4.8++-dev -y >>${LOGFILE} 2>&1
		[ $? -eq 0 ] && ec=0 || ec=1
		[ $ec -eq 0 ] && echo -en $STATUS0 || echo -en $STATUS1
		...
		echo -en " Installing required packages \r"
		sudo apt-get update >>${LOGFILE} 2>&1
		sudo apt-get install unzip curl libboost1.62-dev libboost1.62-tools-dev libboost-mpi-python1.62 libboost-mpi-python1.62-dev libboost1.62-all-dev libminiupnpc-dev libevent-dev libdb4.8-dev libdb4.8++-dev -y >>${LOGFILE} 2>&1
		[ $? -eq 0 ] && ec=0 || ec=1
		[ $ec -eq 0 ] && echo -en $STATUS0 || echo -en $STATUS1
		...
		echo -en " Installing required packages \r"
		[ $? -eq 0 ] && echo -en $STATUS0 || echo -en $STATUS1
	elif [ $osver = "Core" ]; then
		echo -en " Installing required packages \r"
		sudo yum install -y epel-release >>${LOGFILE} 2>&1
		sudo yum install -y unzip curl libtool openssl-devel libevent-devel boost-devel libdb4-devel libdb4-cxx-devel miniupnpc-devel python3 >>${LOGFILE} 2>&1
		[ $? -eq 0 ] && ec=0 || ec=1
		[ $ec -eq 0 ] && echo -en $STATUS0 || echo -en $STATUS1
		...
		#sudo yum update -y >>${LOGFILE} 2>&1
		sudo yum install -y unzip curl libtool compat-openssl10-devel libevent-devel boost-devel libdb4-devel libdb4-cxx-devel miniupnpc-devel python3 --allowerasing >>${LOGFILE} 2>&1
		[ $? -eq 0 ] && ec=0 || ec=1
		echo -en " Installing required packages                 \r"
		[ $ec -eq 0 ] && echo -en $STATUS0 || echo -en $STATUS1
...
function download_wallet() {
	...
	# OS specififc variables
		if [ $osver = "trusty" ]; then
			...
			#download Fedora 29 wallet
			filename="${F29WALLETLINK##*/}"
			filepath=$HOME'/'$filename
			walletlink=${F29WALLETLINK}
		fi
...
function configure_masternode() {
	...
	if ! [ -d "$datadir" ]; then
		...
		if [ $newusermn -eq 1 ]; then
			sudo --user=$newuser mkdir -p ${datadir} 2>>${LOGFILE}
		else
			mkdir -p $datadir 2>>${LOGFILE}
		fi
	...
	echo "addnode=176.53.163.194" >>${coinconf}
	echo "addnode=explorer.proximity.cash" >>${coinconf}
	echo "addnode=176.53.163.191" >>${coinconf}
	echo "addnode=176.53.163.190" >>${coinconf}
	echo "addnode=37.99.123.79" >>${coinconf}
	echo "addnode=185.213.208.238" >>${coinconf}
	echo "addnode=103.108.62.26" >>${coinconf}
	...
	if ! [ "$BLOCKDUMPLINK" = "" ]; then
		if [ -d "${datadir}/blocks/" ]; then
			...
			sudo rm -R ${datadir}/database/ ${datadir}/blocks/ ${datadir}/chainstate/ >>${LOGFILE} 2>&1
			echo -en $STATUS0
		fi
...
function print_result() {
	if [ $setupmn -eq 1 ]; then
		...
		#show instruction to start masternode in local wallet
			...
			echo -en "   ${PURPLE}startmasternode alias false \"mnalias\" ${NC}\n"
			echo
		...
		# post-start status report
			mnstatus=$(${WALLETPATH}/${CLIFILE} masternode debug 2>>${LOGFILE})
			mnstate=$(${WALLETPATH}/${CLIFILE} masternode list full $txhash 2>>${LOGFILE} | grep -oE '(PRE_ENABLED|ENABLED|EXPIRED|WATCHDOG_EXPIRED|NEW_START_REQUIRED|UPDATE_REQUIRED|POSE_BAN|OUTPOINT_SPENT|ACTIVE)' 2>>${LOGFILE})
			logstate=$mnstate
			if [ "$mnstate" = "" ]; then
...
function print_devsupport() {
	...
	echo
	echo -en " ${PURPLE}${TICKER}: Sjthqy7vwqVgvb4SWpw461k2J7dLbnhPsX${NC}\n"
	echo
